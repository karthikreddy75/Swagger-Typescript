...
function generateHook(apis, types, config) {
    ...
    try {
        ...
        const apisCode = apis.reduce((prev, { summary, deprecated, serviceName, queryParamsTypeName, pathParams, requestBody, headerParams, isQueryParamsNullable, isHeaderParamsNullable, responses, method, queryParameters, }) => {
            var _a, _b, _c;
            const hookName = `use${toPascalCase(serviceName)}`;
            const hasPaging = (_a = config.useInfiniteQuery) === null || _a === void 0 ? void 0 : _a.find((name) => name.toLowerCase() === serviceName.toLowerCase() ||
                name.toLowerCase() === hookName.toLowerCase());
            const isGet = hasPaging ||
                method === "get" ||
                ((_b = config.useQuery) === null || _b === void 0 ? void 0 : _b.find((name) => name.toLowerCase() === serviceName.toLowerCase() ||
                    name.toLowerCase() === hookName.toLowerCase()));
            const getParamsString = (override) => ` ${pathParams.length ? `${pathParams.map(({ name }) => name)},` : ""}
      ...
      ${getJsdoc({
          ...
          `;
            ...
            queryFn:({ pageParam }) =>
              fun({
                  ${(_c = queryParameters.find(({ name }) => allowedPageParametersNames.includes(name.toLowerCase()))) === null || _c === void 0 ? void 0 : _c.name}:pageParam,
              }),
              initialPageParam: 1,
        ...
        code += getHooksFunctions({
            hasInfinity: !!((_b = config.useInfiniteQuery) === null || _b === void 0 ? void 0 : _b.length),
        });
        code += `
